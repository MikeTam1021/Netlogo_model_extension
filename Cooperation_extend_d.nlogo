...
to go
  ask turtles [  ;; includes both breeds
    if flip-strat? [ look-around ]
    move
    if flip-strat? [ starving ]
    eat
    reproduce
  ]
  ask patches [
...
to look-around
  if count cooperative-cows-on neighbors > count greedy-cows-on neighbors * ratio-to-become-cooperative and color != red - 1.5 [
    set breed cooperative-cows
    set color red
  ]
end
